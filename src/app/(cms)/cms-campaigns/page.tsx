...
function getStatusBadge(status: CampaignStatus) {
  switch (status) {
    ...
    case "scheduled": return <span className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-blue-50 text-blue-700 rounded-full"><Clock className="w-3 h-3" /> Scheduled</span>;
    case "ended": return <span className="flex items-center gap-1 px-2 py-0.5 text-xs font-medium bg-gray-100 text-gray-500 rounded-full"><CheckCircle className="w-3 h-3" /> Ended</span>;
  ...
  return (
    <div className="max-w-[1400px] mx-auto">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-normal text-gray-800">Campaigns</h1>
      ...
      <div className="flex items-center gap-3 mb-4">
        {["all", "active", "scheduled", "ended", "draft"].map(s => (
          <button key={s} onClick={() => setStatusFilter(s)} className={`px-3 py-1.5 text-sm rounded-full border transition-colors capitalize ${statusFilter === s ? "bg-blue-50 border-blue-200 text-blue-600" : "border-gray-200 text-gray-600 hover:bg-gray-50"}`}>
      ...
      <div className="space-y-3">
        {filteredCampaigns.map(campaign => (
          <div key={campaign.id} className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-sm transition-shadow">
            <div className="flex items-start justify-between">
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-500 ml-8 mb-3">{campaign.description}</p>
                <div className="flex items-center gap-6 ml-8 text-xs text-gray-500">
      ...
      {showCreateModal && (
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center" onClick={() => setShowCreateModal(false)}>
          <div className="bg-white rounded-xl w-full max-w-lg p-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-medium text-gray-800 mb-4">Create new campaign</h2>
            ...
            <div className="flex justify-end gap-3 mt-6">
              ...
              <button className="px-4 py-2 text-sm text-white bg-blue-600 rounded-lg hover:bg-blue-700">Create campaign</button>
            </div>
          </div>
        </div>
      )}
